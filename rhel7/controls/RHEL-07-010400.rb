...
# START_DESCRIBE RHEL-07-010400
  ...
  if command('grep services /etc/sssd/sssd.conf |grep -i nss').exit_status == 0
    describe command('grep memcache_timeout /etc/sssd/sssd.conf | awk \'{print $3}\'') do
      its('stdout') { should cmp <= 86400 }
    end
  end
